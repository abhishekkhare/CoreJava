 ...
 * is roughly equivalent to Vector, except that it is unsynchronized.) The size,
 * isEmpty, get, set, iterator, and listIterator operations run in constant
 * time.
 * 
 ...
 * implementation. Each ArrayList instance has a capacity. The capacity is the
 ...
 * cost. An application can increase the capacity of an ArrayList instance
 ...
 * prevent accidental unsynchronized access to the list: List list =
 ...
 * iterators should be used only to detect bugs. This class is a member of the
 ...
 * @param <E>
 */
public class CustomArrayList<E> extends CustomAbstractList<E> implements CustomList<E>, RandomAccess, Cloneable, java.io.Serializable {
     ...
     */
    public CustomArrayList(CustomCollection<? extends E> c) {
    ...
    public Object clone() {
        try {
            ...
            CustomArrayList<E> v = (CustomArrayList<E>) super.clone();
    ...
    private boolean batchRemove(CustomCollection<?> c, boolean complement) {
     ...
     */
    private class ListItr extends Itr implements CustomListIterator<E> {
        ListItr(int index) {
            super();
            cursor = index;
        }
    ...
    private class SubList extends CustomAbstractList<E> implements RandomAccess {
        private final CustomAbstractList<E> parent;
        private final int parentOffset;
        private final int offset;
        private int size;

        SubList(CustomAbstractList<E> parent,
        ...
        public E set(int index, E e) {
            rangeCheck(index);
            checkForComodification();
            E oldValue = CustomArrayList.this.elementData(offset + index);
            CustomArrayList.this.elementData[offset + index] = e;
            return oldValue;
        }
        ...
        public E get(int index) {
            rangeCheck(index);
            checkForComodification();
            return CustomArrayList.this.elementData(offset + index);
        }
        ...
        public CustomListIterator<E> listIterator(final int index) {
            checkForComodification();
            rangeCheckForAdd(index);

            return new CustomListIterator<E>() {
                int cursor = index;
                int lastRet = -1;
                int expectedModCount = CustomArrayList.this.modCount;
                ...
                public void set(E e) {
                    ...
                    try {
                    	CustomArrayList.this.set(offset + lastRet, e);
                    } catch (IndexOutOfBoundsException ex) {
                        throw new ConcurrentModificationException();
                    }
                }
