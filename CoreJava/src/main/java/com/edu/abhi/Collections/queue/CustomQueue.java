package com.edu.abhi.Collections.queue;
 ...
 * be removed by a call to or poll(). In a FIFO queue, all new elements are
 ...
 * properties. The offer method inserts an element if possible, otherwise
 * returning false. This differs from the Collection.add(java.lang.Object)
 ...
 * "bounded") queues. The remove() and poll() methods remove and return the head
 ...
 * poll() method returns null. The element() and peek() methods return, but do
 * not remove, the head of the queue. The Queue interface does not define the
 * blocking queue methods, which are common in concurrent programming. These
 * methods, which wait for elements to appear or for space to become available,
 * are defined in the java.util.concurrent.BlockingQueue interface, which
 * extends this interface. Queue implementations generally do not allow
 * insertion of null elements, although some implementations, such as
 * LinkedList, do not prohibit insertion of null. Even in the implementations
 * that permit it, null should not be inserted into a Queue, as null is also
 * used as a special return value by the poll method to indicate that the queue
 * contains no elements. Queue implementations generally do not define
 * element-based versions of methods equals and hashCode but instead inherit the
 * identity based versions from class Object, because element-based equality is
 * not always well-defined for queues with the same elements but different
 * ordering properties. This interface is a member of the Java Collections
 * Framework.
 * 
 * 
