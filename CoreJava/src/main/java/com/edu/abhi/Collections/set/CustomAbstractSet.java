...
public abstract  class CustomAbstractSet<E> extends CustomAbstractCollection<E> implements CustomSet<E>{
	...
	public boolean equals(Object o) {
        ...
        if (!(o instanceof CustomAbstractSet))
            return false;
        @SuppressWarnings("rawtypes")
		CustomCollection c = (CustomCollection) o;
        if (c.size() != size())
            return false;
        try {
    ...
    public boolean removeAll(CustomCollection<?> c) {
        ...
        if (size() > c.size()) {
            for (CustomIterator<?> i = c.iterator(); i.hasNext(); )
                modified |= remove(i.next());
        } else {
            for (CustomIterator<?> i = iterator(); i.hasNext(); ) {
