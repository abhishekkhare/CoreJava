 ...
 * also included here for convenience. The List interface provides four methods
 * for positional (indexed) access to list elements. Lists (like Java arrays)
 * are zero based. Note that these operations may execute in time proportional
 * to the index value for some implementations (the LinkedList class, for
 * example). Thus, iterating over the elements in a list is typically preferable
 * to indexing through it if the caller does not know the implementation.
 * 
 ...
 * position in the list. The List interface provides two methods to search for a
 ...
 * remove multiple elements at an arbitrary point in the list. Note: While it is
 * permissible for lists to contain themselves as elements, extreme caution is
 * advised: the equals and hashCode methods are no longer well defined on such a
 * list. Some list implementations have restrictions on the elements that they
 ...
 * specification for this interface. This interface is a member of the Java
...
public interface CustomList<E> extends CustomCollection<E> {
     ...
     */
    boolean addAll(int index, CustomCollection<? extends E> c);
