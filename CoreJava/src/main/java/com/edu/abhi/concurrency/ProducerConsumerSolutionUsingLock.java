...
public class ProducerConsumerSolutionUsingLock {
	public static void main(String[] args) { // Object on which producer and
												// consumer thread will operate
		ProducerConsumerImpl sharedObject = new ProducerConsumerImpl(); // creating
																		// producer
																		// and
																		// consumer
																		// threads
		...
		try {
			...
			if (isAdded) {
				System.out.printf("%s added %d into queue %n", Thread
						.currentThread().getName(), number); // signal consumer
																// thread that,
																// buffer has
																// element now
				System.out.println(Thread.currentThread().getName()
		...
		try {
			...
			if (value != null) {
				System.out.printf("%s consumed %d from queue %n", Thread
						.currentThread().getName(), value); // signal producer
															// thread that,
															// buffer may be
															// empty now
				System.out.println(Thread.currentThread().getName()
